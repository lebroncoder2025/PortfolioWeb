...
export default function HomePage({ siteData = {} }) {
  ...
  return (
    <main style={{ backgroundColor: colors.linen }}>
      {/* Hero Section */}
      {isVisible('hero') && (
        <section className="w-full py-20 px-6 md:py-32" style={{ backgroundColor: colors.cream }}>
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div className="text-center md:text-left">
                {hero.image && (
                  <img src={hero.image} alt="Profile" className="w-40 h-40 md:w-48 md:h-48 mx-auto md:mx-0 rounded-full object-cover mb-8 border-4 shadow-lg" style={{ borderColor: colors.gold }} />
                )}
              </div>
              <div>
                ...
                <p className="text-lg md:text-xl mb-8 leading-relaxed" style={{ color: colors.brown }}>
                  {hero.subtitle || 'Twoje portfolio z zaawansowanym panelem admin'}
                </p>
                <div className="flex gap-3 justify-center md:justify-start flex-wrap">
                  <button onClick={() => navigate('/about')} className="px-8 py-3 rounded-lg font-semibold border-2 transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95" style={{ borderColor: colors.gold, color: colors.gold }}>
                    O mnie
                  </button>
                  ...
                  <button onClick={() => navigate('/contact')} className="px-8 py-3 rounded-lg font-semibold border-2 transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95" style={{ borderColor: colors.gold, color: colors.gold }}>
                    Kontakt
                  </button>
                </div>
              </div>
      ...
      {isVisible('about') && about.content && (
        <section className="w-full py-16 md:py-24 px-6" style={{ backgroundColor: colors.linen }}>
          <div className="max-w-6xl mx-auto">
            <h2 className="text-4xl md:text-5xl font-bold mb-8 text-center" style={{ color: colors.darkGray }}>
              O mnie
            </h2>
            <div className="bg-white rounded-lg p-8 md:p-12 shadow-md border-b-4" style={{ borderColor: colors.gold }}>
              <p className="text-lg leading-relaxed mb-6" style={{ color: colors.brown }}>
                {about.content}
              </p>
              <button onClick={() => navigate('/about')} className="px-6 py-2 rounded-lg font-semibold mt-4" style={{ backgroundColor: colors.gold, color: 'white' }}>
                Czytaj więcej →
              </button>
            </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {services.slice(0, 3).map((service, idx) => (
                <div key={idx} className="bg-white rounded-lg p-8 shadow-md hover:shadow-lg transition-all border-t-4" style={{ borderColor: colors.gold }}>
